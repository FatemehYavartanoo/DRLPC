#Functions related to DRLPC (2019/10/04 version)
#Chages : Alias removal step now extended to the genes with any size!!! (2019/10/14)
...
#DRLPC : Dimention Reduction usign Local Principal Component ################
#Copied from DRLPC_11062019.R(change in output variables from 10202019 version) 

#<input>
#onedata: input data of X variables (0,1,2 coded genotypes) and Y (phenotype) variable with headers
#oneSNPinfo : SNP information data that matches the SNP list of onedata (chrN, rsID, bp)
#vifcut : vif threshold
#CLQcut : CLQ clustering threshold for |r|
#pccut : for principal component inclusion criteria in step 7.
#Yvar : the column name of the Y variable (phenotype)
#itermax : interation limit for high VIF SNP removal procedure in step 5. 
#Klim : the partition limit for alias removal and vif removal

#<output>
#vdata : newly updated  version of onedata after dimension reduction. 
#        Some variables are removed and replaced by LPC variables (Local PC variables by step 3) 
#        extra PC variables (Re-added PC variables (RPC) by step 7)
#LPCinfo : for each LPC variable, original SNPs it is replacing  are listed (ex. SNP1-SNP2-SNP3)
#taglist : vector of tagging variables for each removed/replaced SNPs: the removed SNP is shown in the names for matching tag variables (step 6)
#aliasremoved :  SNPs removed due to complete dependency (step 1)
#removed : final removed SNPs after whole procedure
#RPCind : index of PC variables created and added to the data in step 7


DRLPC=function(onedata, oneSNPinfo, vifcut=20, CLQcut=0.5, pccut=0.8, Yvar="PHENOTYPE", itermax=1000, Klim=300){
  
  #Check later if following variables are declared again! -> remove that redundant lines!!!!!!!!!!!
  SNPsinLPC=NULL
  tagvarlist=NULL
  group=NULL
  grpnames=NULL
  ...
  #put error message if Yvar is not in the data and stop!!!!!!!!!!! 
  ...
  #2.Using CLQD using a cut (0.95, 0.9, 0.8), find clique clusters
 ...
 # CLQout= LDblock[[k]]
  ...
  if(numclst>0){
    for(i in 1:numclst){
      ...
      PCout=PCscores(subset,pccut=pccut, type="PC1") #pccut not used for type=PC1, fix PCscores function later!!!!!
    ...
    remainSNPs2=varnames[-rmInd]   #test data of 200 SNPs : 45 SNPs
  ...
  #65variables including 20 group variable
  
  #update tagSNP list such that list are replaced by PC vars if needed??? Maybe later!!!!!!!!!
  ...
  if(length(removedSNPs)>0){
    ...
    if(length(removedSNPs)>1) rmvPCs=PCscores(rmvgeno,pccut=pccut, type="PCmulti") #test data : 4 PCs with pccut=0.8
    if(dim(rmvPCs)[2]==1) colnames(rmvPCs)="PC1" #2019/02/23
    ...
    if(length(lowr2ind)>0){
      ...
      rmvPCs=rmvPCs[,lowr2ind,drop=FALSE] #bug fix 2020/09/21
      regdata=cbind(dataup,rmvPCs) #bug fix 2020/09/21
      
      #alias check if there are multiple RPCs expected (added in 2020/04/30)
  ...
  if(length(grpnames)>0){ 
   ...
   #   geno1<-genoout[group[[remaingrind[i]]]]
    #  geno1<-dataup[group[[remaingrind[i]]]]
      cov1<-cov(geno1)
      e1<-eigen(cov1)
...
###VIF removal iteration until no change (2019/10/05 updated to be used DRLPC function)###################################
...
###VIF removal using partitioning (2019/10/05 updated to be used DRLPC function)###################################
###2019/10/08 updated to handle large variables!!(VIFlarge, and bulk removal)
###2019/10/20 updated to insert a preprocessing to check if there are aliased SNPs before sending to vif computations
vif.remove=function(dataup, grpnames, Yvar, Klim, itermax, highnumlim=60){
  ...
  #preprocessing if any aliased SNPs exists  (added on 2019/10/20 )###
  ...
  #5. Remove high VIF variables interatively. 
  
  maxvif=max(vifs)
  maxind=which(vifs==max(vifs))[1]
  var1=remainvars[maxind]
  #cornew=cor(dataup[,varnames])
  ...
  while(maxvif>vifcut){
    ...
    #remove the variables with max vif or bulk remove if the # of high vif var > 60
    highvifs=which(vifs>vifcut)
    highnum=length(highvifs)
    if(highnum>highnumlim){ 
      numrmv=floor(highnum*0.1)
      #nth(vifs,numrmv, descending=T, index.return = T)
    ...
    remainvars=setdiff(colnames(dataup),Yvar)  #updated overwritten

    grpnames=setdiff(grpnames,rmvvars)
    
    #if remained variable is less than one SNP break #2020/03/02
...
###partition update + additional removal comparing between partitions (2019/10/05 updated to be used DRLPC function) (alias removal step 5~6)
...
#partition update (2019/10/04 updated to be used DRLPC function) (alias removal step 2~3)
part.update=function(onedata,alremSNPlist, alrmvSNPlist, alremsizes,Klim, Yvar ){
  ...
  if(length(combine)>0){
    ...
    for(k in 1:length(combine)){
      ...
      #for(j in combine[[k]]){
      #  remSNPs=c(remSNPs, alremSNPlist[[j]])
      # 
      #}
...
#find list of group of patitions (2019/10/04 updated to be used DRLPC function, part.update)
...
#alias removal (2019/10/04 updated to be used DRLPC function, part.update)
...
#PC80 scores (2019/06/06 updated to use PCA in FactoMineR package) 
#<input>
...
PCscores=function(indata,pccut=0.8,type=c("PCmulti","PC1")){
  #indmax=5 #maximum number of scores to calculate in PCA!
  indmax=dim(indata)[2]
  ...
  #pc=princomp(indata[,sub],cor=TRUE) #================>change to prcomp PCA for numerical accuracy
...
#PC80 scores (2019/01/24 updated to use for PC1-> option type added)
#<input>
#indata: input data of X variables only
#type : PCmulti=choose PCs according to pccut, PC1=choose first PC
#pccut: threhold for selecting PC variables (minimum PCs explaining variances at least >pccut)
...
PCscores_prcomp=function(indata,pccut=0.8,type=c("PCmulti","PC1")){
  #if(length(colnames(Rdata))==2) return(Wald(rout$beta,rout$invcov))
  
  #ingenodata=Rdata[,2:dim(Rdata)[2]]
  #pheno=Rdata[,1]
  fRmodel=colnames(indata)
  mean_onedata=apply(indata,2,mean)
  max_onedata=apply(indata,2,max)
